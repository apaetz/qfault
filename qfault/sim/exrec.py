...
import qfault.util.counterUtils
import qfault.sim.sim_utils
from qfault.sim.sim_utils import cnot, measX, measZ, prepX, prepZ, rest
...
errorRates = sim.sim_utils.ErrorRates()
...
def findPseudoThresh(pStart=0.0009, pStep=0.0001):
	...
	p = pStart
	while p1 <= p:
		errorRates = sim_utils.ErrorRates()
		errorRates.cnot = p
		errorRates.prepX = errorRates.prepZ = errorRates.measX = errorRates.measZ = 4/15. * errorRates.cnot
		errorRates.rest = 12/15. * errorRates.cnot
		#errorRates.rest = 0
