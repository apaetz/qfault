...
from qfault.counting.convolve import convolve_dict, convolve_counts
...
class KnillScheme(Scheme):
    ...
    def prEgivenAccept(self, eA, eB, noiseModels):
        ...
        generator = SyndromeKeyGenerator(TrivialStablizerCode())
        e = (generator.get_key(eA), generator.get_key(eB))
...
#        inputs = {0: (0,)}
        ...
        prK0 = probability.pr_at_least_k_failures(0, rec.locations(), noiseModels[Pauli.Y], 0)
    ...
    def omega(self, noiseModel, ga):
        ...
        Lambda = 4*prefactor*SymPolyWrapper(sympoly1d([7,0])) + probability.pr_at_least_k_failures(2, self.ed.locations(), noiseModel)
        e = 2.72
        prK0Ga = probability.pr_at_least_k_failures(0, ga.locations(), noiseModel, kMax=0)
        prK0ED = probability.pr_at_least_k_failures(0, self.ed.locations(), noiseModel, kMax=0)
    ...
    def prEgivenS(self, ga, e, s, noiseModels):
        ...
        prE = probability.counts_as_poly(counts, exrec.locations(), noiseModels[Pauli.Y])
...
#        pr = SymPolyWrapper(sympoly1d([n, 0]))
        ...
        inCounts = convolve_counts(inCountsA, 
                                   inCountsA, 
                                   convolve_fcn=convolveInCounts)
        ...
        pr = probability.counts_as_poly(sCounts, rec.locations(pauli), noiseModels[pauli])
